...
if [[ -z "$HEAPDUMP_DIR" ]] ; then
  HEAPDUMP_DIR="$GREMLIN_HOME/heapdump"
fi
...
if [[ "$JAVA_OPTIONS" = "" ]] ; then
    JAVA_OPTIONS="-Xms512m -Xmx4096m -Dfile.encoding=UTF8"
fi
...
start() {
  ...
  if [[ -z "$RUNAS" ]]; then
    ...
    mkdir -p "$HEAPDUMP_DIR" &>/dev/null
    if [[ ! -d "$HEAPDUMP_DIR" ]]; then
      echo ERROR: HEAPDUMP_DIR $HEAPDUMP_DIR does not exist and could not be created.
      exit 1
    fi

    $JAVA -XX:-HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=./$HEAPDUMP_DIR/java_heap.hprof -Dlog4j.configuration=$LOG4J_CONF $JAVA_OPTIONS -cp $CP:$CLASSPATH $GREMLIN_SERVER_CMD "$GREMLIN_YAML" >> "$LOG_FILE" 2>&1 &
    PID=$!
    disown $PID
    echo $PID > "$PID_FILE"
  else
    ...
    su -c "mkdir -p $HEAPDUMP_DIR &>/dev/null"  "$RUNAS"
    if [[ ! -d "$HEAPDUMP_DIR" ]]; then
      echo ERROR: HEAPDUMP_DIR $HEAPDUMP_DIR does not exist and could not be created.
      exit 1
    fi

    su -c "$JAVA -XX:-HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=./$HEAPDUMP_DIR/java_heap.hprof -Dlog4j.configuration=$LOG4J_CONF $JAVA_OPTIONS -cp $CP:$CLASSPATH $GREMLIN_SERVER_CMD \"$GREMLIN_YAML\" >> \"$LOG_FILE\" 2>&1 & echo \$! "  "$RUNAS" > "$PID_FILE"
